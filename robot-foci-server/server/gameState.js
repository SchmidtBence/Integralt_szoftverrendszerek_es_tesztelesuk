...
function updateGameState(state) {
    ...
    // Játékos mozgatása és pályahatárok
    for (let i = 0; i < state.players.length; i++) {
        ...
        player.x = Math.max(playerRadius, Math.min(fieldWidth - playerRadius, player.x));
        player.y = Math.max(playerRadius, Math.min(fieldHeight - playerRadius, player.y));
    }

    // Játékos-játékos ütközés
    ...
    if (state.ball.x < ballRadius || state.ball.x > fieldWidth - ballRadius) {
        state.ball.vx *= -1;
        state.ball.x = Math.max(ballRadius, Math.min(fieldWidth - ballRadius, state.ball.x));
    }
    if (state.ball.y < ballRadius || state.ball.y > fieldHeight - ballRadius) {
        state.ball.vy *= -1;
        state.ball.y = Math.max(ballRadius, Math.min(fieldHeight - ballRadius, state.ball.y));
    }

    // Labda-játékos interakció (összegzett eltolás)
    let totalPushX = 0;
    let totalPushY = 0;
    ...
    for (const player of state.players) {
        ...
        if (dist < minDist && dist > 0) {
            // Rúgás
            switch (player.direction) {
                case 'up': state.ball.vy = -kickPower; break;
                case 'down': state.ball.vy = kickPower; break;
                case 'left': state.ball.vx = -kickPower; break;
                case 'right': state.ball.vx = kickPower; break;
            }

            // Eltolás felhalmozása
